//------------------------------------------------------------------------------------------------------------
//! \file   linear_solvers/Abstract/SweepPattern/SweepPatternUniCartKBA.cpp
//! \brief  Contains implementations and instantiations of methods from SweepPatternUniCartKBA class.
//!
...
//============================================================================================================
//=== MEMBER DEFINITIONS FOR SweepPatternUniCartKBA CLASS ====================================================
//============================================================================================================
...
const std::string SweepOperator<OrdinateFlux>::SweepPatternUniCartKBA::Descriptor( void ) {
...
//--------------------------------------------------------------------------------------------------------
template< class OrdinateFlux >
const std::string SweepOperator<OrdinateFlux>::SweepPatternUniCartKBA::GetDescriptor( void ) const {
...
//------------------------------------------------------------------------------------------------------------
template< class OrdinateFlux >
SweepOperator<OrdinateFlux>::SweepPatternUniCartKBA::~SweepPatternUniCartKBA( void ) {

    PRINT_STATUS( "Executing SweepOperator<%s>::SweepPatternUniCartKBA::%s.\n",
...
//------------------------------------------------------------------------------------------------------------
template< class OrdinateFlux >
void SweepOperator<OrdinateFlux>::SweepPatternUniCartKBA::Print (
...
//------------------------------------------------------------------------------------------------------------
template< class OrdinateFlux >
void SweepOperator<OrdinateFlux>::SweepPatternUniCartKBA::SetParameters (
...
//------------------------------------------------------------------------------------------------------------
template< class OrdinateFlux >
void SweepOperator<OrdinateFlux>::SweepPatternUniCartKBA::Sweep (
     ...
     *
     * NOTE: Diagonals are counted across all angular quadrants.
     */
    int64_t diags_received = 0;
    int64_t diags_computed = 0;
    ...
    # pragma omp parallel if( tid_outer != 0 )
    {
         ...
         ********************************************************************************************/
        {
            ...
            for ( int64_t quad_index : { 0, 1, 2, 3 } ) {
                ...
                if ( rank_source_X == MPI_PROC_NULL ) {
                    ...
                    diags_received += num_diags;
                    # pragma omp flush( diags_received )
                }

                // Sweep pipeline using diagonal wavefront.
                for ( int64_t diag = 1; diag <= num_diags; ++diag ) {

                    // Post receive for incoming boundary data with shift in X dimension.
                    if (    diag <= this->sw_op.nx(1)
                         && rank_source_X != MPI_PROC_NULL
                    ) {
                        // Coordinates of halo cell to receive data into for current quadrant.
                        const int64_t ii = ( signs[0] /* ξ < 0 */ ? this->sw_op.nx(0) + 1 : 0 );
                        const int64_t jj = ( signs[1] /* η < 0 */ ? this->sw_op.nx(1) - diag + 1 : diag );
                    ...
                    // Increment count.
                    if ( diags_received - quad_index * num_diags < diag ) {

                        diags_received += ( diag == this->sw_op.nx(1) ? this->sw_op.nx(0) : 1 );
                        # pragma omp flush( diags_received )
                    }

                    // Post send for boundary data with shift in X dimension.
                    if (    diag >= this->sw_op.nx(0)
                         && rank_dest_X != MPI_PROC_NULL
                    ) {
                        // Wait for boundary computation to finish.
                    # if defined (ENABLE_KBA_WAIT_TIMING)
                        Global::TMR_KBA_comm_wait.Start();
                    # endif

                        while ( diags_computed - quad_index * num_diags < diag ) {

                            nanosleep( &s_sleep_ts, nullptr );

                            # pragma omp flush( diags_computed )
                        }
                    ...
                    # endif
                        ...
                        // Index of cell to send data from with respect to first quadrant.
                        const int64_t j = diag - this->sw_op.nx(0) + 1;

                        // Coordinates of cell to send data from for current quadrant.
                        const int64_t ii = ( signs[0] /* ξ < 0 */ ? 1 : this->sw_op.nx(0) );
                        const int64_t jj = ( signs[1] /* η < 0 */ ? this->sw_op.nx(1) - j + 1 : j );
                ...
                # endif
                    ...
                    while ( diags_computed - quad_index * num_diags < num_diags ) {
        ...
        # endif // if defined (ENABLE_HWLOC) && defined (USE_HWLOC_ALLOC)
            ...
            for ( int64_t diag = 1; diag <= num_diags; ++diag ) {
                ...
                int64_t phase_max [2] = {0,0};
                const int64_t diag_length = this->ComputeDiagLength( diag );

                // Maximum number of cells along diagonal that can be updated without receive operation.
                phase_max[1] = diag_length - ( diag > this->sw_op.nx(1) ? 0 : 1 );

                // Determines whether or not first cell in diagonal should be isolated to enable rapid send.
                if ( diag >= this->sw_op.nx(0) )
                    phase_max[0] = std::min( ((int64_t)(1)), phase_max[1] );

                /* --- Phase 1 computation. --------------------------------------------------------
                 *
                 *  NOTE: Only updates one cell with index i = nx(0) with respect to the first
                 *         quadrant (other quadrants may reflect this value).
                 */
                ...
                /* --- Increment count for send operation. ---------------------------------------- */
                ...
                for ( int64_t cell  = phase_max[0] + 1;
                              cell <= phase_max[1];
                              cell++
                ) {
                for ( int64_t q_blk = 0; q_blk < num_angle_blocks; ++q_blk ) {
                ...
                {
                /*
                 *  Because the condition diag != 1 is used, the timer TMR_KBA_comp_wait records
                 *  only the portion of the wait time between diagonals; i.e., the time spent
                 *  waiting for the first messages to arrive from neighboring ranks is not
                 *  included.
                 */
                # if defined (ENABLE_KBA_WAIT_TIMING)
                    if ( diag != 1 ) {  Global::TMR_KBA_comp_wait.Start();  }
                # endif

                    while ( diags_received - quad_index * num_diags < diag ) {
                ...
                # if defined (ENABLE_KBA_WAIT_TIMING)
                    if ( diag != 1 ) {  Global::TMR_KBA_comp_wait.Stop();  }
                # endif
                }
                ...
                /* --- Phase 3 computation. -------------------------------------------------------
                 *
                 *  NOTE: Only updates one cell with index i = 1 with respect to the first quadrant
                 *        (other quadrants may reflect this value).
                 */

                 if ( phase_max[1] < diag_length ) {
...
//!
//! \brief  Instantiation of SweepOperator<RKDG::OrdinateFlux>::SweepPatternUniCartKBA class.
//!
template class SweepOperator<RKDG::OrdinateFlux>::SweepPatternUniCartKBA;

//!
//! \brief  Instantiation of factory for SweepOperator<RKDG::OrdinateFlux>::SweepPatternUniCartKBA.
//!
template class
SweepOperator<RKDG::OrdinateFlux>::SweepPattern::SweepPatternDerivedFactory<
        SweepOperator<RKDG::OrdinateFlux>::SweepPatternUniCartKBA
    >;


//!
//! \brief  Instantiation of SweepOperator<STDG::OrdinateFlux>::SweepPatternUniCartKBA class.
//!
template class SweepOperator<STDG::OrdinateFlux>::SweepPatternUniCartKBA;

//!
//! \brief  Instantiation of factory for SweepOperator<STDG::OrdinateFlux>::SweepPatternUniCartKBA.
//!
template class
SweepOperator<STDG::OrdinateFlux>::SweepPattern::SweepPatternDerivedFactory<
        SweepOperator<STDG::OrdinateFlux>::SweepPatternUniCartKBA
    >;
