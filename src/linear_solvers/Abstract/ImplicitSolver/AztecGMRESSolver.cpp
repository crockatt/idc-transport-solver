//------------------------------------------------------------------------------------------------------------
//! \file   linear_solvers/Abstract/ImplicitSolver/AztecGMRESSolver.cpp
//! \brief  Contains implementations and instantiations of methods from AztecGMRESSolver class template.
//!
...
//============================================================================================================
//=== MEMBER DEFINITIONS FOR AztecGMRESSolver CLASS ==========================================================
//============================================================================================================
...
const std::string AztecGMRESSolver<AngularFlux>::Descriptor( void ) {
...
//--------------------------------------------------------------------------------------------------------
template< class AngularFlux >
const std::string AztecGMRESSolver<AngularFlux>::GetDescriptor( void ) const {
...
//------------------------------------------------------------------------------------------------------------
//! \brief  Initializes a AztecGMRESSolver object.
//!
...
//------------------------------------------------------------------------------------------------------------
template< class AngularFlux >
AztecGMRESSolver<AngularFlux>::AztecGMRESSolver (
    ...
    sigma_s{ nullptr }
{
    PRINT_STATUS( "Executing AztecGMRESSolver<%s>::%s.\n", typeid(AngularFlux).name(), __func__ )
    ...
    this->illsp = std::shared_ptr<Epetra_Operator>( new ILLSP( *this ) );

    this->epetra_vec_x = std::shared_ptr<Epetra_MultiVector>(
            new Epetra_MultiVector( this->illsp->OperatorDomainMap(), 1 )
        );

    this->epetra_vec_b = std::shared_ptr<Epetra_MultiVector>(
            new Epetra_MultiVector( this->illsp->OperatorRangeMap(), 1 )
        );

    // Setup Epetra_LinearProblem and AztecOO solver.
    this->transport_prob = std::shared_ptr<Epetra_LinearProblem>(
            new Epetra_LinearProblem( this->illsp.get(), this->epetra_vec_x.get(), this->epetra_vec_b.get() )
        );
    ...
    AztecGMRESSolver<AngularFlux>::SetParameters( input_list );
}
...
//------------------------------------------------------------------------------------------------------------
template< class AngularFlux >
AztecGMRESSolver<AngularFlux>::~AztecGMRESSolver ( void ) {
...
//------------------------------------------------------------------------------------------------------------
template< class AngularFlux >
void AztecGMRESSolver<AngularFlux>::Print (
    ...
    PRINT_LOG( "%s%-*s  %s\n", prefix.c_str(),
               Global::col_width, "Solve Type:", this->Descriptor().c_str() )
    ...
    PRINT_LOG( "%s%-20s  %s\n", prefix.c_str(), "Preconditioner:", "none" )
...
//------------------------------------------------------------------------------------------------------------
template< class AngularFlux >
void AztecGMRESSolver<AngularFlux>::SetParameters (
...
//------------------------------------------------------------------------------------------------------------
template< class AngularFlux >
void AztecGMRESSolver<AngularFlux>::SetInitialGuess (
    ...
    // Otherwise use guess.
        ...
        guess->PackExternalVector( *this->epetra_vec_x, 1.0, 0.0 );
    }
}
...
//------------------------------------------------------------------------------------------------------------
template< class AngularFlux >
void AztecGMRESSolver<AngularFlux>::Solve (
    ...
    psi.PackExternalVector( *this->epetra_vec_b, 1.0, 0.0 );
...
# endif // if LOGLEVEL >= 1
    ...
    // Get solution.
    source.UnpackExternalVector( *this->epetra_vec_x );
...
//============================================================================================================
//=== MEMBER DEFINITIONS FOR AztecGMRESSolver::ILLSP CLASS ===================================================
//============================================================================================================
...
//!         AztecGMRESSolver::phi with the appropriate objects.
//!
//! \param[in]  solver          Reference to enclosing solver object.
//------------------------------------------------------------------------------------------------------------
template< class AngularFlux >
AztecGMRESSolver<AngularFlux>::ILLSP::ILLSP (

    const AztecGMRESSolver<AngularFlux> & solver
) :
    enc{ solver }
{

    PRINT_STATUS( "Executing AztecGMRESSolver<%s>::ILLSP::%s.\n", typeid(AngularFlux).name(), __func__ )
    ...
    this->domain_decomp_map = std::shared_ptr<Epetra_Map>(
            new Epetra_Map( (long long int) enc.psi->GetGlobalArrayDim(),
                            (long long int) enc.psi->GetLocalArrayDim(),
                            (int) 0, *this->comm )
        );
}
...
//------------------------------------------------------------------------------------------------------------
template< class AngularFlux >
int AztecGMRESSolver<AngularFlux>::ILLSP::SetUseTranspose ( bool ) {
...
//------------------------------------------------------------------------------------------------------------
template< class AngularFlux >
int AztecGMRESSolver<AngularFlux>::ILLSP::Apply (
...
//------------------------------------------------------------------------------------------------------------
template< class AngularFlux >
int AztecGMRESSolver<AngularFlux>::ILLSP::ApplyInverse (
...
//--------------------------------------------------------------------------------------------------------
template< class AngularFlux >
double AztecGMRESSolver<AngularFlux>::ILLSP::NormInf ( void ) const {
...
//--------------------------------------------------------------------------------------------------------
template< class AngularFlux >
const char * AztecGMRESSolver<AngularFlux>::ILLSP::Label ( void ) const {
...
//--------------------------------------------------------------------------------------------------------
template< class AngularFlux >
bool AztecGMRESSolver<AngularFlux>::ILLSP::UseTranspose ( void ) const {
...
//--------------------------------------------------------------------------------------------------------
template< class AngularFlux >
bool AztecGMRESSolver<AngularFlux>::ILLSP::HasNormInf ( void ) const {
...
//--------------------------------------------------------------------------------------------------------
template< class AngularFlux >
const Epetra_Comm & AztecGMRESSolver<AngularFlux>::ILLSP::Comm ( void ) const {
...
//--------------------------------------------------------------------------------------------------------
template< class AngularFlux >
const Epetra_Map & AztecGMRESSolver<AngularFlux>::ILLSP::OperatorDomainMap ( void ) const {
...
//--------------------------------------------------------------------------------------------------------
template< class AngularFlux >
const Epetra_Map & AztecGMRESSolver<AngularFlux>::ILLSP::OperatorRangeMap ( void ) const {
...
//!
//! \brief  Instantiation of AztecGMRESSolver<RKDG::OrdinateFlux> class.
//!
template class AztecGMRESSolver<RKDG::OrdinateFlux>;

//!
//! \brief  Instantiation of factory for AztecGMRESSolver<RKDG::OrdinateFlux>.
//!
template class
ImplicitSolver<RKDG::OrdinateFlux>::ImplicitSolverDerivedFactory<
        AztecGMRESSolver<RKDG::OrdinateFlux>
    >;


//!
//! \brief  Instantiation of AztecGMRESSolver<STDG::OrdinateFlux> class.
//!
template class AztecGMRESSolver<STDG::OrdinateFlux>;

//!
//! \brief  Instantiation of factory for AztecGMRESSolver<STDG::OrdinateFlux>.
//!
template class
ImplicitSolver<STDG::OrdinateFlux>::ImplicitSolverDerivedFactory<
        AztecGMRESSolver<STDG::OrdinateFlux>
    >;
