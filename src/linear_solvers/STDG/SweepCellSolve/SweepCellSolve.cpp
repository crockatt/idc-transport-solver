...
//!
//! SweepCellSolve::DG_degree_x is set based on the value determined by GetDGDegreeX and
//! SweepCellSolve::DG_degree_t is set based on the value determined by GetDGDegreeT.
//!
...
SweepCellSolve< STDG::OrdinateFlux, SIMD_length >::SweepCellSolve (
    ...
    SweepCellSolveBase<STDG::OrdinateFlux>( enclosing, dt_in ),
    DG_degree_x{ GetDGDegreeX( input_list ) },
    DG_degree_t{ GetDGDegreeT( input_list ) }
{
    PRINT_STATUS( "Executing SweepCellSolve<STDG::OrdinateFlux,%" PRId64 ">::%s.\n", SIMD_length, __func__ )

    this->system_dim = IntPow( this->DG_degree_x + 1, SPACE_DIMS ) * (this->DG_degree_t + 1);
}
...
//------------------------------------------------------------------------------------------------------------
template< int64_t SIMD_length >
SweepCellSolve< STDG::OrdinateFlux, SIMD_length >::~SweepCellSolve ( void ) {
...
//------------------------------------------------------------------------------------------------------------
template< int64_t SIMD_length >
void SweepCellSolve< STDG::OrdinateFlux, SIMD_length >::StoreResult (
...
# if SPACE_DIMS == 1
    ...
    for ( int64_t l = 0; l < SIMD_length; ++l ) {
        ...
        result( idx.q + l, idx.i, d,s) = B[ VecIdx<SIMD_length>(d,s,l) ];
    }}}
...
# elif SPACE_DIMS == 2
    ...
    for ( int64_t l = 0; l < SIMD_length; ++l ) {
        ...
        result( idx.q + l, idx.i, idx.j, d,e,s) = B[ VecIdx<SIMD_length>(d,e,s,l) ];
    }}}}
...
# elif SPACE_DIMS == 3
    ...
    for ( int64_t d = 0; d <= this->DG_degree_x; ++d ) {
    for ( int64_t e = 0; e <= this->DG_degree_x; ++e ) {
    for ( int64_t f = 0; f <= this->DG_degree_x; ++f ) {
    for ( int64_t s = 0; s <= this->DG_degree_t; ++s ) {
    # pragma omp simd aligned( B : SWEEP_SIMD_LEN * SIZEOF_DOUBLE )
    for ( int64_t l = 0; l < SIMD_length; ++l ) {

        result( idx.q + l, idx.i, idx.j, idx.k, d,e,f,s) = B[ VecIdx<SIMD_length>(d,e,f,s,l) ];
    }}}}}
...
# if SWEEP_SIMD_LEN >= (1 << 0)
    template class SweepCellSolve< STDG::OrdinateFlux, (1 << 0) >;
# endif
# if SWEEP_SIMD_LEN >= (1 << 1)
    template class SweepCellSolve< STDG::OrdinateFlux, (1 << 1) >;
# endif
# if SWEEP_SIMD_LEN >= (1 << 2)
    template class SweepCellSolve< STDG::OrdinateFlux, (1 << 2) >;
# endif
# if SWEEP_SIMD_LEN >= (1 << 3)
    template class SweepCellSolve< STDG::OrdinateFlux, (1 << 3) >;
# endif
# if SWEEP_SIMD_LEN >= (1 << 4)
    template class SweepCellSolve< STDG::OrdinateFlux, (1 << 4) >;
# endif
# if SWEEP_SIMD_LEN >= (1 << 5)
    template class SweepCellSolve< STDG::OrdinateFlux, (1 << 5) >;
# endif
# if SWEEP_SIMD_LEN > (1 << 5)
